...
`JavaAudioDeviceModule` (libwebrtc default recorder) at the **`MIC` source, 16 kHz mono** (`SHARED_VOICE_INPUT_RATE_HZ` — the source/rate the game's own voice chat captures with, [ADR-004](../decisions/ADR-004-mic-capture-coexistence.md) round 3) ──▶ `"mic"` PC audio track. The mic track still rides libwebrtc's **software** APM — the audio source is created with default constraints, so AEC/NS/AGC are on (only the media PC disables them, because playback capture is finished audio). What was deliberately dropped is the HAL voice processing tied to `VOICE_COMMUNICATION` — see the coexistence note below.
...
**Concurrent-capture coexistence (ADR-004, found live):** `VOICE_COMMUNICATION` is *privacy-sensitive by default*, and under Android's concurrent-capture policy a privacy-sensitive capture wins priority while every other recording is silenced — turning the cast mic on muted the game's voice chat (PUBG). The fix (round 1 — superseded by the round-3 addendum below) keeps the voice source (its AEC/NS/AGC is tied to it and wanted for viewers) but makes the record itself **privacy-insensitive**: at recording start the ADM's mic `AudioRecord` is swapped for a twin with `setPrivacySensitive(false)` (`MicRecordSubstituter` — the same substitution pattern factory A uses; the fork's builder exposes no privacy-sensitive API). If substitution cannot run, the stock record stays — the mic works as before the fix, logged not failed. A silence monitor (`MicSilenceMonitor` + `AudioRecordingCallback`) surfaces the reverse case — the platform muting *our* capture because another app's recording has priority — as the plain-words `MicState.Silenced`, never a silent failure; it recovers automatically when the winner stops recording. **Addendum (round 2):** the capture is also aligned with the shared HAL path — input rate **16 kHz** (`SHARED_VOICE_INPUT_RATE_HZ`, matching the on-device voice-chat rate; a 48 kHz record on a 16 kHz shared path received raw data = chipmunk at the desktop) and the twin routed to the built-in microphone via `setPreferredDevice` (the platform had split it onto the back mic while the game kept the builtin mic). **Addendum (round 3, user-directed):** the capture source itself is `MIC` (`MicRecordSubstituter.CAPTURE_SOURCE`) — the exact source the game's voice chat uses, so both captures are hosted identically; the voice source kept being split onto the back mic and even the preferred-device request could not move it. Cost: the HAL AEC/NS/AGC tied to the voice source is gone — narrower than it sounds, because the mic track still runs libwebrtc's **software** APM (AEC/NS/AGC on by default via the default constraints), so noise suppression and gain control remain. The echo limitation below was never addressable by the platform chain either in this topology (the mic PC's ADM never plays out the game audio, so no AEC has a far-end reference for the speaker); headphones remain the mitigation.
