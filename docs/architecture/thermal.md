...
- **Phase12 (implemented):** the `adaptive` module — a pure, JVM-tested policy machine (`AdaptiveQualityController`) driven by ~1 Hz sender stats *and* the thermal ladder, wired by `CastService`:
  ...
  - Mechanics: a step applies live without renegotiation (`changeQuality` — capture format reconfig + sender bitrate window + the encoder fps cap). Values are starting hypotheses, re-tuned from Phase 15 measurements (see [features/adaptive-quality.md](../features/adaptive-quality.md)). **Phase16 live finding (2026-09-24):** the fps column of this table was a placebo on device until now — screen capture cannot throttle frame rate, so every cast, including the ladder steps, actually ran at display rate (~61 fps under a 30 fps target, ~30 ms/frame of encoder work) until the fix shipped with [features/performance.md](../features/performance.md). Any thermal number gathered before Phase16 (including anything confirming the table below) was measured through a cast encoding double the intended frame rate; the T1–T4 protocol runs must run on the fixed build.
