...
`ui/settings/SettingsScreen.kt`, entered from the home screen via a "Cast settings" text button (idle and failed states). One screen, every cast default the product has:

- **Quality profile** — one-tap presets (`config/QualityProfile.kt`): `balanced` (1280 px long edge, 30 fps, 4–6 Mbps — the Phase5 values the app shipped with), `sharp` (1920, 30, 8–12), `smooth` (1280, 60, 6–10), `light` (960, 30, 2–3). Picking one applies all of its values.
- **Resolution** (capture long edge: 960 /1280 / 1920 px), **frame rate** (30 / 60 fps) — individually tweakable after a preset. Tweaked values select no preset chip: the honest "custom" state, not a fake match.
- **Bitrate** — *Automatic* (follows the quality profile's window; preset picks restore it) or *manual*: a 2–20 Mbps ceiling slider; the window's floor is half of it (named constant `MANUAL_BITRATE_MIN_FRACTION`, no magic numbers).
- **Audio defaults for the next cast** — "Include game audio" (default on) and "Start with microphone on" (default off — privacy: the mic never streams without an explicit choice, [audio.md](../architecture/audio.md)). Both remain live-toggleable during a cast exactly as in Phases 7/8; the screen says so.

Every change persists **immediately** (`SettingsViewModel` → `CastSettingsStore`); the screen's fixed sub-line states the contract: **changes take effect on the next cast**. Nothing tries to reconfigure a running session — live application of new capture parameters would need capturer restarts or mid-cast renegotiation, which the roadmap explicitly allows deferring ("or live if cheap" — this wasn't cheap, and mid-cast reconfiguration risk isn't worth it before Phase15's reliability matrix). The desktop stays untouched: no new desktop code, no new messages.

### The model and its one conversion point

- `CastSettings` (`config/CastSettings.kt`) is what the settings screen manipulates and what persists. It carries the chosen preset plus the effective values and a `bitrateAuto` flag (UI state for the auto/manual toggle, not a send target).
...
**Emulator (API 34 AVD, uiautomator-driven):** settings screen renders per spec from the home entry; picking **sharp** persisted `profile:sharp / 1920 / 30 / auto 8–12 Mbps` and selected the sharp chip; flipping to **manual bitrate** persisted the derived window (`bitrateAuto:false, 6–12 Mbps` — floor = half the ceiling); **force-stop + relaunch** restored everything on screen (sharp chip, 1920 px, 30 fps, "Max 12 Mbps", auto off — screen state matched the stored document exactly); the **mic toggle** persisted (`mic:true`). One honest correction from the drive: the settings screen scrolls — with manual bitrate active, the audio rows sit below the fold on a phone-sized viewport (fine, but worth knowing for Phase13 polish).
...
- No resolution above the physical screen is offered as a choice: `CaptureSize` keeps the capture native below the target, so `sharp` on an 800×1280 tablet changes only fps/bitrate there. A per-device filtered choice list would need `DisplaySize` at the settings screen — deferred until a real device needs it.
- Thermal profiles (Phase11) will reuse `QualityProfile` and apply values the settings screen did not pick; the label derivation stays the display's source of truth.
