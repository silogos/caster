...
Implemented in: **Phase8** (see [roadmap](../development/roadmap.md)). Status: **implemented; JVM 138/138 green; verified live on device (2026-09-20)** — mic offers answered on their own pc, mic stream rendered at the receiver, on/off toggle cycles exercised, simultaneous operation with game audio observed. **2026-09-23 fix ([ADR-004](../decisions/ADR-004-mic-capture-coexistence.md)):** the cast mic no longer silences the casted game's voice chat (PUBG) — see *Concurrent-capture coexistence*; the matrix's main item (PUBG voice chat active + clean cast mic, round 3 build) is **verified on device 2026-09-23**, the reverse-direction silence case and the echo sanity remain for Phase 15. The remaining Phase 8 acceptance item is the user's **audible listen** (see *Verification*).

## What is implemented

### The `mic` PC (factory B, ADR-003 amended by [ADR-004](../decisions/ADR-004-mic-capture-coexistence.md))

`MicCastSession` (`webrtc/` module) owns the second PeerConnection: microphone only, on its own `PeerConnectionFactory` with the **stock `JavaAudioDeviceModule`** recording at the **`MIC` source, 16 kHz mono** (`SHARED_VOICE_INPUT_RATE_HZ` — the source/rate the game's own voice chat captures with, ADR-004 round 3) → Opus mono. libwebrtc's **software** APM (AEC/NS/AGC) stays on: the audio source is created with default constraints — only the media PC disables them, because playback capture is finished audio ([audio.md](../architecture/audio.md)). Negotiated over the same WebSocket with the `pc: "mic"` discriminator; no protocol changes — signaling already carried the discriminator since Phase 4.
...
**Fallback superseded (by round 3 below):** the planned one-builder-call fallback — `setAudioSource(MIC)` (non-sensitive by default) — did not wait for a matrix failure; the device experiment promoted it to the shipped source itself. Note: this ROM discards app logcat (Phase 7 constraint), so the monitor's diagnostics are for other devices/emulator; on-device verification of this fix rides on the game's own mic indicator and the desktop listen.
...
**Device round 3 (user-directed):** round 2's preferred-device request did not move the routing — the platform still split the `VOICE_COMMUNICATION` record onto the back mic. The capture source itself is now **`MIC`** (`MicRecordSubstituter.CAPTURE_SOURCE` — shared by the ADM builder, the twin, and the silence monitor), the exact source the game's voice chat uses, so both captures are hosted identically (same source, same 16 kHz, same builtin mic). Cost: the HAL AEC/NS/AGC tied to the voice source is gone — narrower than it sounds: the mic track still rides libwebrtc's **software** APM (AEC/NS/AGC on by default via the default constraints), so noise suppression and gain control remain, and the input is now exactly what the game's own voice chat records (same source, rate, and mic — the desktop hears the input the player's teammates hear). The echo limitation below stands — it was never addressable by the platform chain either in this topology (the mic PC's ADM never plays out the game audio, so no AEC has a far-end reference for the speaker) — headphones remain the mitigation ([ADR-004 round-3 addendum](../decisions/ADR-004-mic-capture-coexistence.md)).
