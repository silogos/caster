...
- **The >10-min long-cast proof, landed as a side effect:** the second measurement cast ran **75 minutes** (user-ended cleanly with `bye`; the 10-minute-killer fix from session C held) — sender encoded 96,571 frames with **zero drops**, receiver decoder drops 0.07 %, stable 30 fps at cool by the end, rtt p50 6 ms throughout. The longest continuous cast recorded on this rig so far; T5's 30-min OBS session remains its own (video-output-side) case.
