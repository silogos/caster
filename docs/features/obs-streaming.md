...
- **Follow-up on the rotation fix, also found live:** the first re-test killed the app — `ScreenCapturerAndroid.changeCaptureFormat` releases the virtual display and calls `MediaProjection.createVirtualDisplay` again, and Android 14+ forbids a second `createVirtualDisplay` on one projection (SecurityException). The project now owns the capturer: `capture/ScreenCapturer.kt` (faithful port of the pinned 1.3.8 class) whose format change **resizes** the single display in place — which also de-crashes the adaptive-quality steps (same call, never exercised on this device before). Re-test with the new build: **the user's portrait→landscape walk passed on the device (2026-09-21)** — the stream flips and the letterbox follows; the session's own logs couldn't be captured for the record because the tablet was unplugged between runs (the on-device walk remains the user's eyes-on evidence, to be re-recorded in Phase15's matrices).
