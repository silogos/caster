...
- The **30+ min OBS Window Capture session**: a clean, correctly-proportioned, smooth feed — the window stays where the user placed it (letterboxing by default), a portrait/landscape rotation re-fits the bars without moving the window, and "Match window to video" works as the on-demand snap. The receiver's side is verified; the session is the user's eyes-on acceptance (roadmap Phase15's matrices will formally record it).
